...
class_name GaeaNodeComposeRect
...
func _get_description() -> String:
	return "Composes %d [code]Vector2[/code]s into [code]%s[/code]." % [_get_arguments_list().size(), _get_rect_type_name()]
...
func _get_output_port_display_name(_output_name: StringName) -> String:
	return "Composed %s" % _get_rect_type_name()
